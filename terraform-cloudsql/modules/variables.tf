...
variable "fl_app_env" {
    ...
    default = "eng"
    validation {
      condition     = contains(["eng", "dev", "test", "prod"], var.fl_app_env)
      error_message = "Valid values for var: fl_app_env are (eng,dev,test,prod)."   
    }
}

variable "fl_data_classification" {
    type = string
    description = "This is used for determining the classifiction of data"
    default = "internal"
    validation {
      condition     = contains(["internal", "public", "restricted", "secret"], var.fl_data_classification)
      error_message = "Valid values for var: fl_data_classification) are (internal,public,restricted,secret)."   
    }
}

variable "fl_data_sensitivity" {
    type = string
    ...
    default = "none"
    validation {
      condition     = contains(["p1", "p2", "p3", "none"], var.fl_data_sensitivity)
      error_message = "Valid values for var: fl_data_sensitivity are (p1,p2,p3,none)."   
    }
}

variable "fl_app_carid" {
    type = string
...
variable "fl_lifecycle_changenumber" {
    ...
    validation {
      condition     = substr(var.fl_lifecycle_changenumber , 0, 3) == "CHG" || substr(var.fl_lifecycle_changenumber , 0, 4) == "EMER"
      error_message = "Valid values for fl_lifecycle_changenumber  must start with CHG or EMER."   
              }
}
 
variable "fl_net_designation" {
    type = string
    ...
    default = ""
    validation {
      condition     = contains(["application","backup","critical-infrastructure","database","enterprise","internet","internet-outbound","management","orchestration","scrubbing","security","storage","support"], var.fl_net_designation)
...
variable "fl_account_type" {
    type = string
    description = "This is used for determining the roles created in the account."
    default = "platform"
    validation {
        condition     = contains(["infrastructure", "security", "platform"], var.fl_account_type)
        error_message = "Valid values for var: fl_account_type are (infrastructure, security, platform)."   
    }
}

variable "fl_provisioner_carid" {
  type = string
  description = "The CAR ID associated with the project or team that owns the tool which provisioned the resource. This could be the same as fl-owner-carid,or a different CAR ID if provisioned on behalf of the fl-owner-carid."
  default = "sample_car_id"

}

 
variable "fl_provisioner_repo" {
  type = string
  description = "This is a required tag and must be the URI to the repository which contains the automation code."
  default = "sample_repo"
}

variable "fl_provisioner_workspace" {
  type = string
